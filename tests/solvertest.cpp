...
void solve(double yhfov, double xhfov, int Nx, int Ny, double sep, double panstart, double panend, double tiltstart, double tiltend, double r1, int x1, int y1, double theta1m, double phi1m, double r2, int x2, int y2, double theta2m, double phi2m, int argc, char** argv) {
    GFLAGS_NAMESPACE::ParseCommandLineFlags(&argc, &argv, true);
    google::InitGoogleLogging(argv[0]);
    double theta0 = EIGEN_PI / 6;
    double phi0 = EIGEN_PI / 6;
    double theta0p = EIGEN_PI / 6;
    double phi0p = EIGEN_PI / 6;
    Problem problem;
    ...
    double theta1p = 2 * EIGEN_PI * theta1m / 3 / (tiltend - tiltstart);
    double phi1p = EIGEN_PI * ((phi1m / (panend - panstart)) - 0.5);
    double theta2p = 2 * EIGEN_PI * theta2m / 3 / (tiltend - tiltstart);
...
int main(int argc, char** argv) {
    solve(EIGEN_PI/5, EIGEN_PI/4, 346, 260, 1, 0, 300, 0, 450, 1000, -45, 60, 100, 300, 1000, 50, 75, 270, 325, argc, argv);
    return 0;
}